...
export async function load({ locals, params: { eventId } }) {
	...
	const pattern = getPattern(dev ? Math.random().toString() : eventId)

	const sessionId = locals.session.get(eventId)?.id

	if (!sessionId) return { event, pattern, session: undefined }
	...
	if (!session) return { event, pattern, session: undefined }
